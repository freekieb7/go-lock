...
func (h *Handler) HandleDeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.JSONResponse(w, http.StatusInternalServerError, response.APIResponse{
			...
			Message: shared.ErrFailedUserRetrieve,
	...
	// Delete the user
	if err := h.AccountService.DeleteUserByID(ctx, user.ID); err != nil {
		h.Logger.ErrorContext(ctx, shared.ErrFailedUserDelete, "error", err)
		response.JSONResponse(w, http.StatusInternalServerError, response.APIResponse{
			Code:    http.StatusInternalServerError,
			Message: shared.ErrFailedUserDelete,
			Status:  shared.StatusError,
		})
		return
	}

	h.Logger.InfoContext(ctx, "User deleted successfully", "user_id", user.ID, "email", user.Email)
