...
func TestAuthorizeByCode(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest("GET", fmt.Sprintf(`/auth/oauth/authorize?response_type=code&client_id=%s&redirect_uri=%s&audience=%s&scope=offline_access`, client.Id, client.RedirectUris[0], resourceServer.Uri), nil)
